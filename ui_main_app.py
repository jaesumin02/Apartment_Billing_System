...
class MainApp(ctk.CTk):
    ...
    def show_billing(self):
        ...
        ctk.CTkButton(actions, text="Show Receipt", width=130, command=self.show_receipt).pack(side="left", padx=4)
    ...
    def show_receipt(self):
        payment_id = self.get_selected_payment_id()
        if not payment_id:
            messagebox.showwarning("Select", "Please select a payment to view receipt.", parent=self)
            return
        row = self.payment_model.get(payment_id)
        if not row:
            messagebox.showwarning("Not Found", "Payment not found.", parent=self)
        ...
        try:
            ...
            try:
                ...
                print("Failed to show receipt error dialog:", e, file=sys.stderr)
    ...
    def edit_payment(self):
        ...
        if dlg.saved:
            ...
            messagebox.showinfo("Payment Updated", "Payment status has been updated.", parent=self)
            # If the payment is now marked as paid, show the receipt dialog
            try:
                updated = self.payment_model.get(payment_id)
                if updated and (updated.get("status") or "").lower() == "paid":
                    try:
    ...
    def mark_maintenance_in_progress(self):
        ...
        all_requests = self.maintenance_model.all()
        pending_requests = [m for m in all_requests if (m["status"] or "").lower() == "pending"]
        
        if not pending_requests:
            messagebox.showinfo("No Pending", "No pending maintenance requests to mark in progress.", parent=self)
            return
        
        req_id = pending_requests[0]["request_id"]
        
        self.maintenance_model.update_status(req_id, "In Progress")
        self.load_maintenance()
        self.log_action("Maintenance In Progress", f"request_id={req_id}")
        messagebox.showinfo("Updated", f"Request #{req_id} marked as In Progress.", parent=self)

    def mark_maintenance_completed(self):
        all_requests = self.maintenance_model.all()
        in_progress_requests = [m for m in all_requests if (m["status"] or "").lower() == "in progress"]
        
        if not in_progress_requests:
            messagebox.showinfo("No In Progress", "No in-progress maintenance requests to mark completed.", parent=self)
            return
        req_id = in_progress_requests[0]["request_id"]
        ...
        messagebox.showinfo("Completed", f"Request #{req_id} marked as completed.", parent=self)

    def edit_maintenance(self):
        req_id = self.get_selected_maintenance_id()
        if not req_id:
            messagebox.showwarning("Select", "Please select a maintenance request to edit.", parent=self)
            return
        ...
        dlg = MaintenanceDialog(self, self.staff_model)
        dlg.title("Edit Maintenance Request")
        # Pre-fill with existing data
        dlg.tid_e.delete(0, tk.END)
        dlg.tid_e.insert(0, str(m.get("tenant_id") or ""))
        dlg.desc_e.delete(0, tk.END)
        dlg.desc_e.insert(0, m.get("description") or "")
        dlg.prio_cmb.set(m.get("priority") or "Low")
        dlg.fee_e.delete(0, tk.END)
        dlg.fee_e.insert(0, str(m.get("fee") or 0))
        dlg.staff_cmb.set(m.get("staff") or "")
        
        self.wait_window(dlg)
        if dlg.saved:
            r = dlg.result
            self.maintenance_model.update(
                req_id, r["tenant_id"], r["description"], r["priority"], r["fee"], r["staff"]
            )
            self.load_maintenance()
            self.log_action("Edit Maintenance", f"request_id={req_id}")
            messagebox.showinfo("Saved", "Maintenance request updated.", parent=self)

    def delete_maintenance(self):
        req_id = self.get_selected_maintenance_id()
        if not req_id:
            messagebox.showwarning("Select", "Please select a maintenance request to delete.", parent=self)
            return
        m = self.maintenance_model.get(req_id)
        if not m:
            messagebox.showwarning("Not Found", "Maintenance request not found.", parent=self)
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete maintenance request #{req_id}?\nThis can be restored later.", parent=self):
            self.maintenance_model.delete(req_id)
            self.load_maintenance()
            self.log_action("Delete Maintenance", f"request_id={req_id}")
            messagebox.showinfo("Deleted", "Maintenance request deleted.", parent=self)

    def show_deleted_maintenance(self):
        self.current_view = "deleted_maintenance"
        self.clear_body("Deleted Maintenance")
        frame = self.body_frame
        frame.grid_rowconfigure(0, weight=0)
        frame.grid_rowconfigure(1, weight=1)
        frame.grid_columnconfigure(0, weight=1)
        ...
        action_frame = ctk.CTkFrame(filter_box, fg_color="transparent")
        action_frame.pack(side="left", padx=12, pady=12)
        ctk.CTkButton(action_frame, text="Restore Selected", width=140, command=self.restore_maintenance, fg_color="#33ff99").pack(side="left", padx=4)
        ctk.CTkButton(action_frame, text="Back to Active", width=120, command=self.show_maintenance).pack(side="left", padx=4)
        ...
        self.deleted_maint_tree = ttk.Treeview(table_box, columns=cols, show="headings", style="WhiteBlueprint.Treeview")
        for c in cols:
            self.deleted_maint_tree.heading(c, text=c.replace("_", " ").title())
            self.deleted_maint_tree.column(c, minwidth=100, stretch=True, anchor="w")
        self.deleted_maint_tree.grid(row=0, column=0, sticky="nsew", padx=8, pady=8)

        vsb = ttk.Scrollbar(table_box, orient="vertical", command=self.deleted_maint_tree.yview)
        self.deleted_maint_tree.configure(yscrollcommand=vsb.set)
        vsb.grid(row=0, column=1, sticky="ns", padx=(0,8), pady=8)

        self.load_deleted_maintenance()

    def load_deleted_maintenance(self):
        if not hasattr(self, "deleted_maint_tree"):
            return
        for r in self.deleted_maint_tree.get_children():
            self.deleted_maint_tree.delete(r)
        for m in self.maintenance_model.all_including_deleted():
            if m.get("deleted") == 1:
                self.deleted_maint_tree.insert(
                    "",
                    tk.END,
                    values=(
                        m["request_id"],
    ...
    def get_selected_deleted_maintenance_id(self):
        if not hasattr(self, "deleted_maint_tree"):
            return None
        sel = self.deleted_maint_tree.selection()
        if not sel:
            return None
        item = self.deleted_maint_tree.item(sel[0])
        vals = item.get("values") or []
        if not vals:
            return None
        try:
    ...
    def restore_maintenance(self):
        req_id = self.get_selected_deleted_maintenance_id()
        if not req_id:
            messagebox.showwarning("Select", "Please select a maintenance request to restore.", parent=self)
            return
        m = self.maintenance_model.get_deleted(req_id)
        if not m:
            messagebox.showwarning("Not Found", "Deleted maintenance request not found.", parent=self)
            return
        
        if messagebox.askyesno("Confirm Restore", f"Restore maintenance request #{req_id}?", parent=self):
            self.maintenance_model.restore(req_id)
            self.load_deleted_maintenance()
            self.load_maintenance()
            self.log_action("Restore Maintenance", f"request_id={req_id}")
            messagebox.showinfo("Restored", "Maintenance request restored.", parent=self)
